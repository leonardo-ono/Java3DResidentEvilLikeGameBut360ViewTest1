...
public class Triangle extends Polygon {
    ...
    public void draw(Graphics2D g) {
        ...
        g.drawString(id + " path:" + pathDistance 
                + " rc:" + onlyRaycast, (int) x, (int) y);
    ...
    public void getBarycenter(Point2D v) {
        double px = 0.33 * (walls[0].x1 + walls[1].x1 + walls[2].x1);
        double py = 0.33 * (walls[0].y1 + walls[1].y1 + walls[2].y1);
        v.setLocation(px, py);
    }
        ...
        if (current.contains(body.x, body.y)) {
            ...
            for (Wall wall : this.getWalls()) {
                if ((!wall.isBlocking() || wall.canRaycastPassThrough()) 
                        && wall.ptSegDist(body.x, body.y) <= body.radius) {
